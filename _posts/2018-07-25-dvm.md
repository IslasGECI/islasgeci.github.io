...
_dvm_ es una aplicación que emula control de versiones para archivos binarios de datos.
Este _post_ presenta una historia de usuario de _dvm_.
...
La instrucción anterior descarga la versión más nueva sin modificar tu copia local.
Para actualizar tus archivos de datos:
```
> dvm update
```

## Actualiza sólo un archivo
Los comandos `pull` y `update` aceptan la opcion `--filename` para indicar el nombre del archivo que se desea actualizar.
```
...
```
El directorio contiene dos achivos: `foo.bin` y `bar.bin`. El comando `status` busca en `dvm.json` el nombre de los archivos que encuentra en el directorio (y que no se encuentran en `.dvmignore`).
Dado que `dvm.json` está vació, `status` despliega:
```
> dvm status
    ?   foo.bin
    ?   bar.bin
...
Vemos que en la carpeta `.dvm/` hay tres archivos: dos versiones de `foo.bin` y una versión de `bar.bin`.
El nuevo archivo `md5_0AD066A5D29F3F2A2A1C7C17DD082A79.dvm` todavía no se ha subido al servidor.
Para subir la nueva versión de `foo.bar` usamos nuevamente el comando `push`.
