...
Agregamos semillas a las pruebas: al inicio de los archivos de pruebas, al inicio de los `describe` y hasta en cada uno de los `it`.
...
``` R
get_rand_state <- function() { # Declaramos la función.
  get0(".Random.seed", envir = .GlobalEnv, inherits = FALSE) # Obtenemos el estado global.
}
```
Con la segunda función podemos regresar al estado de `.Random.seed` que guardamos:
``` R
set_rand_state <- function(state) { # Declaramos función.
  if (!is.null(state)) { # Inicialicamos if.
    assign(".Random.seed", state, envir = .GlobalEnv, inherits = FALSE) # Regresar al estado que se guardó.
  }
}
```
...
Por ejemplo:
```R
my_f <- function() { # Declaramos función.
  old_state <- get_rand_state() # Declaramos variable.
  on.exit(set_rand_state(old_state)) # Aseguramos que fijamos el estado de la variable.
}
```
...
- Sabemos que al fijar una semilla tenemos el mismo valor de `.Random.seed`, aunque no es el único participante en la generación de números aleatorios.
También está el `RNGkind`.
- El problema aún lo tenemos.
Las pruebas tienen dependencia entre cómo las corremos y hasta en el orden en el que las corremos.
